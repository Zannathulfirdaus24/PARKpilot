...
import { activeReservation, bookings } from "../data/dummy";
    ...
    <UserLayout>
      <div className="mb-6 flex items-start justify-between gap-4">
        <div>
          ...
          <h1 className="text-2xl font-bold tracking-tight sm:text-3xl">{user?.name.split(" ")[0]} 👋</h1>
        </div>
      </div>
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="relative overflow-hidden rounded-2xl gradient-primary p-6 text-primary-foreground shadow-xl shadow-primary/30">
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <h2 className="mt-1 text-xl font-bold">{activeReservation.lot}</h2>
            <p className="text-sm text-primary-foreground/85">Slot {activeReservation.slot} · {activeReservation.time}</p>
          </div>
          <div className="rounded-xl bg-white/15 px-4 py-3 text-center backdrop-blur">
            <p className="text-[10px] uppercase tracking-wider text-primary-foreground/80">Time left</p>
            <p className="text-2xl font-bold tabular-nums">{activeReservation.remaining}</p>
          </div>
        </div>
      ...
      <div className="mt-6 grid grid-cols-2 gap-3 md:grid-cols-3">
        <StatCard label="Bookings this month" value="12" delta="+3 vs last" icon={<CalendarRange className="h-5 w-5" />} tone="info" />
        <StatCard label="Total spending" value="$486" delta="+$62" icon={<Wallet className="h-5 w-5" />} tone="success" />
        <StatCard label="Avg. rating given" value="4.8" icon={<TrendingUp className="h-5 w-5" />} tone="warning" />
      </div>
        ...
        {bookings.slice(0, 3).map((b) => (
          <Card key={b.id} className="flex flex-wrap items-center justify-between gap-3">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="font-medium">{b.lot}</p>
                <p className="text-xs text-muted-foreground"><Clock className="mr-1 inline h-3 w-3" />{b.date} · {b.time}</p>
              </div>
            </div>
            <div className="flex items-center gap-3">
