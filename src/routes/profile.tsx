...
import { CreditCard, LogOut, Mail, Phone, Car, Pencil } from "lucide-react";
    ...
    <UserLayout title="Profile">
      <div className="grid gap-6 lg:grid-cols-[1fr_1.4fr]">
        <Card className="text-center">
          <img src={user.avatar} alt="" className="mx-auto h-24 w-24 rounded-full object-cover ring-4 ring-primary/20" />
          <h2 className="mt-4 text-xl font-bold">{user.name}</h2>
          <p className="text-sm text-muted-foreground">Premium Member · Since 2025</p>
          <Button variant="outline" size="sm" className="mt-4"><Pencil className="h-4 w-4" /> Edit Profile</Button>
          <div className="mt-6 grid grid-cols-3 gap-2 border-t border-border pt-5 text-center">
            <div><p className="text-xl font-bold">42</p><p className="text-xs text-muted-foreground">Bookings</p></div>
            <div><p className="text-xl font-bold">$486</p><p className="text-xs text-muted-foreground">Spent</p></div>
            <div><p className="text-xl font-bold">4.9</p><p className="text-xs text-muted-foreground">Rating</p></div>
          </div>
        </Card>
        ...
        <div className="space-y-4">
          ...
          <Button variant="outline" onClick={() => nav({ to: "/login" })} className="w-full text-destructive">
