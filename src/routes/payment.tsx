...
const methods = [
  ...
  { id: "wallet", label: "Wallet", icon: Wallet, desc: "ParkPilot balance: $42" },
];

function PaymentPage() {
  const [sel, setSel] = useState("cc");
      ...
      <div className="grid gap-6 lg:grid-cols-[1.4fr_1fr]">
        ...
        <Card className="h-fit">
          ...
          <div className="mt-4 space-y-2 text-sm">
            <div className="flex justify-between"><span className="text-muted-foreground">Slot</span><span>{draft.slot ?? "A-14"}</span></div>
            <div className="flex justify-between"><span className="text-muted-foreground">Duration</span><span>{draft.hours ?? 2}h</span></div>
            <div className="flex justify-between"><span className="text-muted-foreground">Subtotal</span><span>${(draft.price ?? 60) * (draft.hours ?? 2)}</span></div>
            <div className="flex justify-between"><span className="text-muted-foreground">Fees & tax</span><span>$10</span></div>
            <div className="mt-2 flex justify-between border-t border-border pt-3 text-lg font-bold"><span>Total</span><span className="text-primary">${total}</span></div>
          </div>
          <Button
            loading={loading}
            className="mt-5 w-full"
            onClick={() => { setLoading(true); setTimeout(() => nav({ to: "/success" }), 1200); }}
          >
